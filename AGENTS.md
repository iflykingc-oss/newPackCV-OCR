...
- **核心升级**: RapidOCR(ONNX)主引擎 + 自适应5策略预处理 + 多尺度检测 + 布局感知KV提取 + LLM后验证
...
| model_extract | `graphs/nodes/model_extract_node.py` | agent | **增强提取**：布局感知KV提取(bbox→同行配对)+11字段规则引擎(中英双语)+LLM后验证纠错 | - | `config/model_extract_llm_cfg.json` |
| qa_answer | `graphs/nodes/qa_answer_node.py` | agent | 语义问答（5段结构化分析） | - | `config/qa_answer_llm_cfg.json` |
...
## 规则引擎增强
- **12个提取字段**：brand, product_name, specification, production_date, shelf_life, manufacturer, ingredients, standard, batch_number, license_number, storage_condition, expiry_date
